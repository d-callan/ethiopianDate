...
#' @alias toGregorian.Date
#' @alias toGregorian.character
#' @export
toGregorian <- function(ethiopianDate, ...) UseMethod('toGregorian')
...
toGregorian.Date <- function(ethiopianDate) {
...
#' @export
#' @importFrom lubridate as_date
toGregorian.character <- function(ethiopianDate, format = '%Y-%m-%d') {
  toGregorian(lubridate::as_date(ethiopianDate, format = format))
}
...
ethiopianToGregorian <- function(year = numeric(), month = numeric(), date = numeric()) {
  firstDay = firstDayEthiopian(year)
...
#' @alias toEthiopian.Date
#' @alias toEthiopian.character
#' @export
toEthiopian <- function(gregorianDate, ...) UseMethod('toEthiopian')
...
#' @importFrom lubridate day
#' @importFrom lubridate as_date
toEthiopian.Date <- function(gregorianDate) {
...
#' @export
#' @importFrom lubridate as_date
toEthiopian.character <- function(gregorianDate, format = '%Y-%m-%d') {
  toEthiopian(lubridate::as_date(gregorianDate, format = format))
}
...
gregorianToEthiopian <- function(year = numeric(), month = numeric(), date = numeric()) {
  ...
  firstDay = firstDayEthiopian(year - 8)

  until <- sum(gregorianMonths[1:month])
  until <- until + date
